  ...
  public  class InBoundService
    {
      ...
      public static void UpdateInOut(decimal taskno,  decimal groupno)
      {
          ...
          using (TransactionScope ts = new TransactionScope())
          {
              using (Entities entity = new Entities())
              {
                  var taskList = (from item in entity.T_PRODUCE_POKE where item.TASKNUM == taskno && item.GROUPNO==groupno select item).ToList();
                  foreach (var task in taskList)
                  {
                      ...
                      outTask.BARCODE = ItemService.GetItemByCode(outTask.CIGARETTECODE).BIGBOX_BAR;
