   ...
   public class SortTroughService
    {
       public static List<T_PRODUCE_SORTTROUGH> GetTrough(int troughtype, int cigarettetype)
       {
           ...
           {
               var query = from item in dataentity.T_PRODUCE_SORTTROUGH where item.TROUGHTYPE == troughtype && item.CIGARETTETYPE == cigarettetype && item.CIGARETTECODE!=null && item.STATE == "10" orderby item.SEQ select item ;
               return query.ToList();
           }
       }
