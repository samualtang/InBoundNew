...
group by billcode) y
     ...
     order by sortnum, packtasknum, cigtype, cigseq, SYNSEQ";
        ...
        static Dictionary<String, List<V_PRODUCE_PACKAGEINFO>> needInfoList = new Dictionary<String, List<V_PRODUCE_PACKAGEINFO>>();


        static object lockobj = new object();
        public static List<V_PRODUCE_PACKAGEINFO> GetNeedInfo(decimal packageno, Entities en, decimal maxcallbacknum)
        {
            ...
            lock (lockobj)
            {
                if (!needInfoList.ContainsKey(key))
                {
                    //获取包装机视图
                    var needInfo = (from item in en.V_PRODUCE_PACKAGEINFO where (item.EXPORT == export1 || item.EXPORT == export2) && item.TASKNUM >= maxcallbacknum orderby item.TASKNUM select item).ToList();
                    needInfoList.Add(key, needInfo);
                }
            }
            return needInfoList[key];
        ...
        public void CallBackTBJ2(decimal packageno)
        {
            using (Entities en = new Entities())
            {
                ...
                //en.Configuration.ValidateOnSaveEnabled = false;
                //获取当前包装机最大条烟流水号
                decimal maxcallbacknum;
                if (en.T_PACKAGE_CALLBACK.Where(c => c.EXPORT == packageno).Count() > 0)
                {
                    maxcallbacknum = en.T_PACKAGE_CALLBACK.Where(x => x.EXPORT == packageno).Max(x => x.SORTNUM);
                }
                else
                {
                    maxcallbacknum = 0;
                }
                var cALLBACKs = (from item in en.T_PACKAGE_CALLBACK where item.EXPORT == packageno && item.SORTNUM >= maxcallbacknum select item).ToList();
                decimal maxCigNum = 1;
                decimal maxSortnum = 0;
                if (cALLBACKs.Any())
                {
                    maxSortnum = maxcallbacknum;
                    //maxSortnum = cALLBACKs.Max(a => a.SORTNUM);//获取最大的任务号
                ...
                maxCigNum = (from item in en.T_PACKAGE_CALLBACK where item.EXPORT == packageno select item.CIGNUM).Count() > 0 ? ((from item in en.T_PACKAGE_CALLBACK where item.EXPORT == packageno select item.CIGNUM).Max() + 1) : 1;
                decimal export1 = 0, export2 = 0;
                if (packageno == 1 || packageno == 2)
                {
                    export1 = 1;
                ...
                cALLBACKs = null;
                //获取包装机视图
                var needInfo = (from item in en.V_PRODUCE_PACKAGEINFO
                                where (item.EXPORT == export1 || item.EXPORT == export2) && item.TASKNUM >= maxcallbacknum
                ...
                try
                {
                    ...
                    foreach (var region in regioncode)//单个车组循环
                    {
                        ...
                        routCPagNum = (from items in en.T_PACKAGE_TASK where (items.PACKAGENO == export1 || items.PACKAGENO == export2) && items.REGIONCODE == region.regioncode select items.PACKTASKNUM).Distinct().Count();
                        foreach (var item in oneRegion)//循环单个车组的数据
                        {
                            ...
                            if (!item.BILLCODE.Equals(billcode))//存入新的订单号 ,一个订单插入一次数据
                            {
                                ...
                                UNNORMALPACKAGENUM = GetBillPackNum(bill, 2);//异型烟总包数  
                                bill = null;
                                //System.GC.Collect() ;
                            }
        ...
        decimal GetBillPackNum(List<T_PACKAGE_TASK> bill, int flag)
        {
            //var bill = (from item in en.T_PACKAGE_TASK where item.BILLCODE == billcode orderby item.PACKAGESEQ, item.CIGSEQ select item).ToList();
            var grouppack = bill.Select(a => new { packtasknum = a.PACKTASKNUM }).Distinct().ToList();/// .GroupBy(a => new { packtasknum = a.PACKTASKNUM }).ToList();//一共多少包
            int index = 0;
        ...
        /// </summary>
        /// <returns></returns>
        public static int GetFmCig()
        {
            using (Entities et = new Entities())
            {
                var query = et.T_PACKAGE_TASK.Where(x => x.CIGARETTECODE.Length == 8).ToList();
                foreach (var item in query)
                {
                    item.CIGARETTECODE = item.CIGARETTECODE.Substring(1, 7);
                    et.SaveChanges();
                }
        ...
        public static void UpdateTBJMsg()
        {
            using (Entities et = new Entities())
            {
                var query = (from item in et.T_PACKAGE_CALLBACK select new { billCode = item.BILLCODE, packageNo = item.EXPORT }).Distinct().GroupBy(x => x.billCode).Select(x => new { bcode = x.Key, count = x.Count() });
                var billList = query.Where(x => x.count > 1).Select(x => x.bcode).ToList();
                if (billList.Count > 0)
                {

                    var blist = (from item in et.T_PACKAGE_CALLBACK where billList.Contains(item.BILLCODE) select item).ToList();

                    foreach (var bcode in billList)
                    {
                        var templist = blist.Where(x => x.BILLCODE == bcode).ToList();
                        var count = templist.Where(x => x.PACKAGESEQ == 1).Select(x => new { export = x.EXPORT, packageseq = x.PACKAGESEQ }).Distinct().ToList();
                        var orderCount = templist.Select(x => new { export = x.EXPORT, orderpackNum = x.ORDERPACKAGENUM }).Distinct().Sum(x => x.orderpackNum);
                        var minpackageno = templist.Select(x => x.EXPORT).Min();
                        var maxpakageseq = templist.Max(x => x.PACKAGESEQ);

                        foreach (var b in templist)
                        {
                            if (count.Count() > 1)
                            {
                                if (b.EXPORT != minpackageno)
                                {
                                    b.PACKAGESEQ = orderCount - b.ORDERPACKAGENUM + b.PACKAGESEQ;
                                }

                                b.ORDERPACKAGENUM = orderCount;
                            }
                        }
                    }
                    et.SaveChanges();
