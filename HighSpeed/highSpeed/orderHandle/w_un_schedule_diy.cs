...
{
    public partial class w_un_schedule_diy : Form
    {
        ...
        private void seek()
        {
            ...
            String strsql = "with lst as (SELECT regioncode,sum(t.orderquantity) as qty,COUNT(*)as cuscount from t_produce_order t WHERE unstate='新增'group BY t.regioncode order by t.regioncode ) select rownum,lst.* from lst";
                    ...
                    if (codestr != "")
                    {
                        ...
                        if (MsgBoxResult == DialogResult.Yes)
                        {
                            ...
                            for (int i = 0; i < len; i++)
                            {
                                ...
                                sqlpara[2] = new OracleParameter("p_ErrMsg", OracleType.VarChar, 100);

                                sqlpara[1].Direction = ParameterDirection.Output;
                                sqlpara[2].Direction = ParameterDirection.Output;
                                Db.ExecuteNonQueryWithProc("P_UN_DIY_SCHEDULE", sqlpara);// 修改前的存储过程 P_PRODUCE_SCHEDULE
