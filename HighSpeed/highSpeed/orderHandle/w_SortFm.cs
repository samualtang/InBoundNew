    ...
    public partial class w_SortFm : Form
    {
        ...
        private void btnVid_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                Db.Open();
                double usDataNum = Convert.ToDouble(Db.ExecuteScalar("select sum(orderquantity) from t_produce_task ") == null ? 0 : Db.ExecuteScalar("select sum(orderquantity) from t_produce_task "));//上位总数量
                double usNormalNum = Convert.ToDouble(Db.ExecuteScalar("select sum(pokenum) from t_produce_poke ") == null ? 0 :Db.ExecuteScalar("select sum(pokenum) from t_produce_poke ") ) ;//上位常规烟数量
                double usUnNormalNum = Convert.ToDouble(Db.ExecuteScalar("select sum(pokenum) from t_un_poke ")== null ? 0 :Db.ExecuteScalar("select sum(pokenum) from t_un_poke "));//上位异型烟数量
                double dsDataNum = Convert.ToDouble(Db.ExecuteScalar("select sum(quantity) from  kesheng.v_produce_packageinfo ")== null ? 0 :Db.ExecuteScalar("select sum(quantity) from  kesheng.v_produce_packageinfo "));//下位总数量
                double dsNormalNum = Convert.ToDouble(Db.ExecuteScalar("select sum(pokenum) from  kesheng.v_produce_pokeseq ")== null ? 0 :Db.ExecuteScalar("select sum(pokenum) from  kesheng.v_produce_pokeseq "));//下位常规烟数量
                double dsUnNormalNum = Convert.ToDouble(Db.ExecuteScalar("select sum(quantity) from kesheng.v_un_pokeseq ") == null ? 0 : Db.ExecuteScalar("select sum(quantity) from kesheng.v_un_pokeseq "));//下位异型烟数量
