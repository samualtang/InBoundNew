...
{
    public partial class w_reschedule_alone : Form
    {
        ...
        private void seek()
        {
            ...
            String str = "select regioncode,sum(e.quantity) as qty,count(distinct e.billcode)as cuscount from t_produce_orderline e join  t_produce_order r on r.billcode=e.billcode where r.state='新增' and e.cigarettecode in (select h.cigarettecode from t_produce_sorttrough h where h.cigarettetype =20 and h.troughtype=10 and h.state=10)group by r.regioncode";
                    ...
                    if (codestr != "")
                    {
                        ...
                        if (MsgBoxResult == DialogResult.Yes)
                        {
                            ...
                            for (int i = 0; i < len; i++)
                            {
                                ...
                                Db.ExecuteNonQueryWithProc("P_PRODUCE_PRE_SCHEDULE_ALONE", sqlpara);// 修改前的存储过程 P_PRODUCE_SCHEDULE
        ...
        void Sort()
        {
            ...
            Db.Open();
            Db.ExecuteNonQueryWithProc("P_PRODUCE_SCHEDULE_ALONE", sqlpara);//更改前存储过程  P_PRODUCE_updatesortnum
            //MessageBox.Show(date);
            //MessageBox.Show(code[i]+"订单数据接收完成!", "提示", MessageBoxButtons.OK, MessageBoxIcon.Information);
