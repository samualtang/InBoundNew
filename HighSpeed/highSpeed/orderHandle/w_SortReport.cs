        ...
        void Seek() 
        {
            ...
            switch (CmbAddress.SelectedIndex) 
            {
                case 0:
                    ...
                    break;
                case 1:
                    str = " where billcode like 'CS%'";
                    break;
                case 2:
                    str = " where billcode like 'ZZ%'";
                    break;
                case 3:
                    str = " where billcode like 'XT%'";
                    break;
            }
            string sql = "select f.cigarettename 品牌,decode(f.allowsort,'分拣','常规烟','非标','异型烟') 类型,sum(f.quantity) 数量 from v_produce_packageinfo f " +
