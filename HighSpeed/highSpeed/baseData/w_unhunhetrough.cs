...
{
    public partial class w_unhunhetrough : Form
    {
        ...
        private void seek()
        {
           // string linenum = this.box_sortline.SelectedValue+"";
            ...
            String strsql = "SELECT tmp.* FROM(select rownum as num, id,linenum,troughnum,troughdesc,decode(sign(machineseq-4),-1,'混合道','单独道')as troughtype" +
                            ...
                            "from highspeed.t_produce_sorttrough t where  cigarettetype='40' " + tmp +
            ...
            int total = int.Parse(DataPublic.ExecuteScalar("SELECT count(*) FROM t_produce_sorttrough where 1=1 and cigarettetype='40' " + tmp).ToString());
            Bind(strsql);
        ...
        private void btn_amend_Click(object sender, EventArgs e)
        {
            ...
            if (count > 0)
            {
                ...
                sign = "1";
                win_trough_handle trough_handle = new win_trough_handle(sign,amend_id,"40","");
