   ...
   public  class OpcGroup
    {
       ...
       public void Refresh()
       {
           lock (this)
           {
               ...
               {
                   throw OpcDa.Client.Com.Interop.CreateException("IOPCAsyncIO2.Refresh", e);
               }
           }
       }
       ...
       private OpcRcw.Da.OPCITEMDEF[] GetOPCITEMDEFs(OpcItem[] input)
       {
           ...
           if (input != null)
           {
               ...
               for (int ii = 0; ii < input.Length; ii++)
               {
                   ...
                   output[ii].bActive =  1;
                   output[ii].vtRequestedDataType = (short)VarEnum.VT_EMPTY;//(short)OpcDa.Client.Com.Interop.GetType(input[ii].ReqType);
