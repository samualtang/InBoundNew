...
namespace SortingControlSys.PubFunc
{
    class DataBase
    {
        ...
        public const string connectionString1 = "server=CHUNFEN;database=hnxyw;uid=sa;pwd=sa;";
        #endregion
        ...
        private static string connStr = "Data Source=(DESCRIPTION=(ADDRESS=(PROTOCOL=TCP)(HOST=10.73.196.44)(PORT=1521))(CONNECT_DATA=(SERVICE_NAME=orADB)));User Id=highspeed;Password=admin;";
        ...
        private void Conn()
        {
            try
            {
                OraConn = new OracleConnection(PublicFun.connect);
            }
            catch (Exception ex)
            {
                throw ex;
        ...
        public void Open()
        {
            ...
            {
                //log.Write("connection not null but is close");
                try
                {
        ...
        public void ExecuteNonQueryWithProc(string sqlSpName, SqlParameter[] dbParams)
        {
            try
            {
        ...
        public bool ExecuteSqlTrans(ArrayList sqlList)
        {
            ...
            try
            {
                ...
                {
                    if (!DataPublic.ISSQL(sqlList[i].ToString()))
                    {
                        //回滚
                        innerTransaction.Rollback();
                        return false;
