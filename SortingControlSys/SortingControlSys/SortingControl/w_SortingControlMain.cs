                ...
                if (flag == 2)//0：已取走， 1：已写入
                {
                    while (!ProducePokeService.CheckExistPreSendTask(sortgroupno2, 12) && ProducePokeService.CheckExistPreSendTask(sortgroupno2, 10))
                    {
                        ...
                        for (int i = 1; i <= 4; i++)
                        {
                            ...
                            writeLog.Write("plc地址:" + plclist[(int)((i - 1) * 16 + 2 * (sortgroup - 1))] + "读取组:" + sortgroupno2 + " 主皮带:" + i + " 合流任务号:" + taskno + "；plc地址：" + plclist[(int)((i - 1) * 16 + 2 * (sortgroup - 1) + 1)] + " 抓取数量:" + zqqty);
                            updateListBox("plc地址:" + plclist[(int)((i - 1) * 16 + 2 * (sortgroup - 1))] + "读取组:" + sortgroupno2 + " 主皮带:" + i + " 合流任务号:" + taskno + "；plc地址：" + plclist[(int)((i - 1) * 16 + 2 * (sortgroup - 1) + 1 )] + " 抓取数量:" + zqqty);
